...
class ContactosModel {
  constructor() {
    ...
    this.db.run(
      "CREATE TABLE IF NOT EXISTS contactos (name TEXT, email TEXT, mensaje TEXT, ip TEXT, fecha TEXT, pais TEXT, id INTEGER PRIMARY KEY AUTOINCREMENT)",
      (err) => {
        if (err) {
          console.error(err.message);
        }
  ...
  crearContacto(name, email, mensaje, ip, fecha, pais){
    return new Promise((resolve, reject)=>{
        const sql = `INSERT INTO contactos (name, email, mensaje, ip, fecha, pais) VALUES (?,?,?,?,?,?)`;
        this.db.run(sql, [name, email, mensaje, ip, fecha, pais], function (err){
            if (err){
                console.error(err.message);
                reject(err);
            }
